...
- `docs/` — every other HTML document, published to GitHub Pages:
  ...
  - `docs/module3/quiz-app-tutorial.html` — Module 3: hands-on tutorial for turning a finished problem-set folder into a double-click student quiz page **from scratch**, by prompting an AI agent directly rather than using a pre-built skill, at three levels of prompt detail. Deliberately ships with no `app/` starting point (see `module3/` below) — the exercise is designing the approach, not applying one. Unlike Module 4, this no longer covers the teacher aggregator page.
