...
```bash
python3 .claude/skills/build-quiz-app/scripts/analyze_folder.py <problem-set-folder>
```
...
```bash
mkdir -p <output>/app
cp .claude/skills/build-quiz-app/templates/quiz.html <output>/app/quiz.html
cp .claude/skills/build-quiz-app/templates/report-aggregator.html <output>/app/report-aggregator.html
cp .claude/skills/build-quiz-app/templates/build_bundle.py <output>/app/build_bundle.py
```
...
- If no browser tool is available, at minimum grep each bundle for
  `window.QUIZ_DATA` and spot-check with `Read` that a couple of the
