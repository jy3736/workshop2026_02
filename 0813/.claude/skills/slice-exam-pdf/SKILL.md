...
`build-quiz-app`, spot-check with `Read`:
...
- On a **共同科目 (數學/國文/英文) paper with a 參考公式 preamble** (trap
  #9): `Read` Q1 specifically and confirm it's the real first question, not
...
- On **any question whose stem opens with a brace-grouped system of
  ...
  `Read` both that question *and the one immediately before it* — confirm
