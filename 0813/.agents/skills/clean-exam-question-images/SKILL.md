...
a handful of results afterward — see "Verification" at the end.
...
After running either or both passes, open a small sample back (don't just
...
- **Scan the whole folder for residual bleed**, not just the spot-checked
  ...
  【以下空白】 layout — open it to confirm — but a block flush against the
